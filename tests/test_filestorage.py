...
def vcr_fixture(request):
    with test_config_container.create_vcr().use_cassette('filestorage_{name}.yml'.format(name=request.function.__name__)):
        yield


@pytest.fixture(scope='module')
def vcn_and_subnet(runner, config_file, config_profile, network_client):
    with test_config_container.create_vcr().use_cassette('filestorage_vcn_and_subnet_fixture.yml'):
        # create VCN
    ...
    with test_config_container.create_vcr().use_cassette('filestorage_vcn_and_subnet_fixture_cleanup.yml'):
...
def file_system(filestorage_client, runner, config_file, config_profile):
    with test_config_container.create_vcr().use_cassette('filestorage_file_system_fixture.yml'):
        params = [
            'file-system', 'create',
    ...
    with test_config_container.create_vcr().use_cassette('filestorage_file_system_fixture_cleanup.yml'):
        params = [
            'file-system', 'delete',
...
def mount_target(filestorage_client, vcn_and_subnet, runner, config_file, config_profile):
    with test_config_container.create_vcr().use_cassette('filestorage_mount_target_fixture.yml'):
    ...
    with test_config_container.create_vcr().use_cassette('filestorage_mount_target_fixture_cleanup.yml'):
        params = [
            'mount-target', 'delete',
            '--mount-target-id', mount_target_id,
            '--force'
