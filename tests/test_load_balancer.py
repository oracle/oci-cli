...
def load_balancer(runner, config_file, config_profile, vcn_and_subnets):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_fixture_lb.yml'):
    ...
    with test_config_container.create_vcr().use_cassette('test_load_balancer_fixture_lb_delete.yml'):
        params = [
...
def backend_set(runner, config_file, config_profile, load_balancer):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_fixture_backend_set.yml'):
    ...
    with test_config_container.create_vcr().use_cassette('test_load_balancer_fixture_backend_set_delete.yml'):
        params = [
            'backend-set', 'delete',
            '--load-balancer-id', load_balancer,
...
def backend(runner, config_file, config_profile, load_balancer, backend_set):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_fixture_backend.yml'):
    ...
    with test_config_container.create_vcr().use_cassette('test_load_balancer_fixture_backend_delete.yml'):
        params = [
            'backend', 'delete',
            '--load-balancer-id', load_balancer,
            '--backend-set-name', backend_set,
...
def certificate(runner, config_file, config_profile, load_balancer, key_pair_files):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_fixture_certificate.yml'):
    ...
    with test_config_container.create_vcr().use_cassette('test_load_balancer_fixture_certificate_delete.yml'):
        # delete cert
        params = [
            'certificate', 'delete',
            '--load-balancer-id', load_balancer,
...
def test_load_balancer_operations(runner, config_file, config_profile, load_balancer):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_lb_operations.yml'):
        # list
        params = [
            'load-balancer', 'list',
            '-c', util.COMPARTMENT_ID
...
def test_certificate_operations(runner, config_file, config_profile, load_balancer, certificate):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_cert_operations.yml'):
        params = [
            'certificate', 'list',
            '--load-balancer-id', load_balancer
        ]
...
def test_backend_set_operations(runner, config_file, config_profile, load_balancer, backend_set):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_backend_set_operations.yml'):
...
def test_backend_operations(runner, config_file, config_profile, load_balancer, backend_set, backend):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_backend_operations.yml'):
        # fixture handles create / delete
        params = [
            'backend', 'list',
            '--load-balancer-id', load_balancer,
...
def test_listener_operations(runner, config_file, config_profile, load_balancer, backend_set, certificate):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_listener_operations.yml'):
        # create listener
...
def test_listener_with_connection_timeout_operations(runner, config_file, config_profile, load_balancer, backend_set):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_listener_with_connection_timeout_operations.yml'):
...
def test_load_balancer_health_operations(runner, config_file, config_profile, load_balancer):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_lb_health_operations.yml'):
        params = [
...
def test_backend_set_health_operations(runner, config_file, config_profile, load_balancer, backend_set):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_backend_set_health_operations.yml'):
        params = [
...
def test_backend_health_operations(runner, config_file, config_profile, load_balancer, backend_set, backend):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_backend_health_operations.yml'):
        params = [
            'backend-health', 'get',
            '--load-balancer-id', load_balancer,
            '--backend-set-name', backend_set,
...
def test_health_checker_operations(runner, config_file, config_profile, load_balancer, backend_set):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_health_checker_operations.yml'):
        params = [
...
def test_list_lb_shapes(runner, config_file, config_profile):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_lb_shapes.yml'):
        params = [
            'shape', 'list',
            '-c', util.COMPARTMENT_ID
        ]
...
def test_list_lb_protocols(runner, config_file, config_profile):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_lb_protocols.yml'):
        params = [
            'protocol', 'list',
            '-c', util.COMPARTMENT_ID
        ]
...
def test_list_lb_policy(runner, config_file, config_profile):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_lb_policy.yml'):
        params = [
            'policy', 'list',
            '-c', util.COMPARTMENT_ID
        ]
...
def test_load_balancer_operations_with_waiters(runner, config_file, config_profile, vcn_and_subnets, key_pair_files):
    with test_config_container.create_vcr().use_cassette('test_load_balancer_ops_with_waiters.yml'):
        subnet_ocid_1 = vcn_and_subnets[1]
        subnet_ocid_2 = vcn_and_subnets[2]
