...
RESOURCE_DETAILS_FILE_LOCATION="./autoscaling_example/resource.json"
...
ASC_CONFIG_ID=$(oci autoscaling configuration create --compartment-id $COMPARTMENT_ID --policies file://autoscaling_example/policies.json --resource "$RESOURCE_INPUT" --query 'data.id' --raw-output)
