...
def cancel_long_term_backup(ctx, from_json, wait_for_state, max_wait_seconds, wait_interval_seconds, long_term_backup_id, if_match):
    ...
    if isinstance(long_term_backup_id, str) and len(long_term_backup_id.strip()) == 0:
...
This option is a JSON list with items of type RecoveryServiceSubnetInput.  For documentation on RecoveryServiceSubnetInput please see our API reference: https://docs.oracle.com/en-us/iaas/api/#/en/databaserecovery/20210216/datatypes/RecoveryServiceSubnetInput.""" + custom_types.cli_complex_type.COMPLEX_TYPE_HELP)
