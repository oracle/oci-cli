...
@pytest.mark.generated
def test_change_service_gateway_compartment(cli_testing_service_client, runner, config_file, config_profile):
    if not cli_testing_service_client.is_api_enabled('core', 'ChangeServiceGatewayCompartment'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config_file = os.environ['OCI_CLI_CONFIG_FILE']
    if 'USE_TESTING_SERVICE_CONFIG' in os.environ:
        try:
            config_str = cli_testing_service_client.get_config('core', 'VirtualNetwork', 'ChangeServiceGatewayCompartment')
            config = json.loads(config_str)
            key_file_content = config['keyFileContent']
            with open('tests/resources/keyfile_for_test_change_service_gateway_compartment.pem', 'w') as f:
                f.write(key_file_content)
            with open('tests/resources/config_for_test_change_service_gateway_compartment', 'w') as f:
                f.write('[ADMIN]\n')
                f.write('user = ' + config['userId'] + '\n')
                f.write('fingerprint = ' + config['fingerprint'] + '\n')
                f.write('tenancy = ' + config['tenantId'] + '\n')
                f.write('region = ' + config['region'] + '\n')
                f.write('key_file = tests/resources/keyfile_for_test_change_service_gateway_compartment.pem\n')
            runner.invoke(oci_cli.cli, ['setup', 'repair-file-permissions', '--file', 'tests/resources/config_for_test_change_service_gateway_compartment'])
            runner.invoke(oci_cli.cli, ['setup', 'repair-file-permissions', '--file', 'tests/resources/keyfile_for_test_change_service_gateway_compartment.pem'])
            config_file = 'tests/resources/config_for_test_change_service_gateway_compartment'
        except vcr.errors.CannotOverwriteExistingCassetteException:
            pass
        except Exception as e:
            print(e)
    ...
    root_command_name = oci_cli.cli_util.override('virtual_network_root_group.command_name', 'virtual-network')
    resource_group_command_name = oci_cli.cli_util.override('service_gateway_group.command_name', 'service_gateway')
    request_containers = cli_testing_service_client.get_requests(service_name='core', api_name='ChangeServiceGatewayCompartment')
    for i in range(len(request_containers)):
        request = request_containers[i]['request'].copy()
        done = False
        params = []
        while not done:
            # force all details param names to have lower case first letter for consistency with Java models
            param_name = 'ChangeServiceGatewayCompartmentDetails'
            details = request.pop(param_name[0].lower() + param_name[1:])
            for key in details:
                request[key] = details[key]
                override = util.variable_name_override(key)
            ...
            request, cleanup = generated_test_request_transformers.transform_generated_test_input('core', 'ChangeServiceGatewayCompartment', request)
            ...
            if not params:
                params = util.get_command_list(
                    root_command_name,
                    resource_group_command_name,
                    oci_cli.cli_util.override('change_service_gateway_compartment.command_name', 'change-compartment'),
                    True
                )

            if not params:
                raise ValueError(
                    'Failed to find CLI command "oci {} {} {}" for given operation: core, ChangeServiceGatewayCompartment. '
                    'This usually happens because generated commands have been manually re-arranged in code for better user '
                    'experience. To allow this test to find the proper command, please add an entry to MOVED_COMMANDS in '
                    'services/<spec_name>/tests/extend_test_<your_service_name>.py to map ({}, {}, {}) to the syntax '
                    'for the new command. If the file does not exist for your service, please create one. You can refer the '
            ...
            try:
                ...
                if os.path.exists('tests/resources/keyfile_for_test_change_service_gateway_compartment.pem'):
                    os.remove('tests/resources/keyfile_for_test_change_service_gateway_compartment.pem')
                if os.path.exists('tests/resources/config_for_test_change_service_gateway_compartment'):
                    os.remove('tests/resources/config_for_test_change_service_gateway_compartment')
                if cleanup:
                    try:
                        next(cleanup)
                    except StopIteration:
