...
<b>Oracle AWS Key Resource:</b>&nbsp;&nbsp;The Oracle AWS Key Resource is used to register and manage a AWS Key within Oracle Cloud Infrastructure (OCI)."""), short_help=cli_util.override('oracle_db_azure_vault.oracle_db_azure_vault_root_group.short_help', """Oracle Database MultiCloud Data Plane Integration"""))
...
@oracle_db_azure_vault_group.command(name=cli_util.override('oracle_db_azure_vault.replicate_oracle_db_azure_vault.command_name', 'replicate'), help=u"""Replicate the Oracle DB Azure Vault resource to target region. \n[Command Reference](replicateOracleDbAzureVault)""")
@cli_util.option('--oracle-db-azure-vault-id', required=True, help=u"""The [OCID] of the Oracle DB Azure Vault resource.""")
@cli_util.option('--target-region', required=True, help=u"""The Destination region where Oracle GCP Keyring resource will replicate.""")
@cli_util.option('--action', type=custom_types.CliCaseInsensitiveChoice(["CREATE", "DELETE"]), help=u"""Action for replicate Resource.""")
@cli_util.option('--if-match', help=u"""For optimistic concurrency control. In the PUT or DELETE call for a resource, set the `if-match` parameter to the value of the etag from a previous GET or POST response for that resource. The resource will be updated or deleted only if the etag you provide matches the resource's current etag value.""")
@cli_util.option('--wait-for-state', type=custom_types.CliCaseInsensitiveChoice(["ACCEPTED", "IN_PROGRESS", "WAITING", "NEEDS_ATTENTION", "FAILED", "SUCCEEDED", "CANCELING", "CANCELED"]), multiple=True, help="""This operation asynchronously creates, modifies or deletes a resource and uses a work request to track the progress of the operation. Specify this option to perform the action and then wait until the work request reaches a certain state. Multiple states can be specified, returning on the first state. For example, --wait-for-state ACCEPTED --wait-for-state CANCELED would return on whichever lifecycle state is reached first. If timeout is reached, a return code of 2 is returned. For any other error, a return code of 1 is returned.""")
@cli_util.option('--max-wait-seconds', type=click.INT, help="""The maximum time to wait for the work request to reach the state defined by --wait-for-state. Defaults to 1200 seconds.""")
@cli_util.option('--wait-interval-seconds', type=click.INT, help="""Check every --wait-interval-seconds to see whether the work request has reached the state defined by --wait-for-state. Defaults to 30 seconds.""")
...
@json_skeleton_utils.json_skeleton_generation_handler(input_params_to_complex_types={})
@cli_util.wrap_exceptions
def replicate_oracle_db_azure_vault(ctx, from_json, wait_for_state, max_wait_seconds, wait_interval_seconds, oracle_db_azure_vault_id, target_region, action, if_match):
    ...
    _details = {}
    _details['targetRegion'] = target_region

    if action is not None:
        _details['action'] = action

    client = cli_util.build_client('dbmulticloud', 'oracle_db_azure_vault', ctx)
    result = client.replicate_oracle_db_azure_vault(
        oracle_db_azure_vault_id=oracle_db_azure_vault_id,
        replicate_oracle_db_azure_vault_details=_details,
        **kwargs
    )
    if wait_for_state:
