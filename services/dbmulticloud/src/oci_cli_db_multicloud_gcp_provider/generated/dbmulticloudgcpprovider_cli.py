...
<b>Oracle AWS Key Resource:</b>&nbsp;&nbsp;The Oracle AWS Key Resource is used to register and manage a AWS Key within Oracle Cloud Infrastructure (OCI)."""), short_help=cli_util.override('db_multicloud_gcp_provider.db_multicloud_gcp_provider_root_group.short_help', """Oracle Database MultiCloud Data Plane Integration"""))
...
@oracle_db_gcp_key_ring_group.command(name=cli_util.override('db_multicloud_gcp_provider.replicate_oracle_db_gcp_key_ring.command_name', 'replicate'), help=u"""Replicate Oracle GCP Key Ring details to target region. \n[Command Reference](replicateOracleDbGcpKeyRing)""")
@cli_util.option('--oracle-db-gcp-key-ring-id', required=True, help=u"""The [OCID] of the Oracle DB GCP Key-Ring resource.""")
...
@json_skeleton_utils.json_skeleton_generation_handler(input_params_to_complex_types={})
@cli_util.wrap_exceptions
def replicate_oracle_db_gcp_key_ring(ctx, from_json, wait_for_state, max_wait_seconds, wait_interval_seconds, oracle_db_gcp_key_ring_id, target_region, action, if_match):
    ...
    client = cli_util.build_client('dbmulticloud', 'db_multicloud_gcp_provider', ctx)
    result = client.replicate_oracle_db_gcp_key_ring(
        oracle_db_gcp_key_ring_id=oracle_db_gcp_key_ring_id,
        replicate_oracle_db_gcp_key_ring_details=_details,
        **kwargs
    )
    if wait_for_state:
