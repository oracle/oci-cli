...
class TestNodePoolCreateUpdate(unittest.TestCase):
    ...
    def test_nodepool_create(self):
        result = util.invoke_command(['ce', 'node-pool', 'create', '--size', '1', '--nsg-ids', 'nsgs', '--kms-key-id', 'kmskey', '--is-pv-encryption-in-transit-enabled', 'true', '--placement-configs', 'config'])
        assert 'Error: Missing option(s)' in result.output

    def test_nodepool_update(self):
        result = util.invoke_command(['ce', 'node-pool', 'update', '--size', '1', '--nsg-ids', 'nsgs', '--kms-key-id', 'kmskey', '--is-pv-encryption-in-transit-enabled', 'true', '--placement-configs', 'config', '--node-source-details', '{"foo", "bar"}', '--ssh-public-key', 'key', '--node-shape', 'shape', '--node-metadata', '{"foo", "bar"}'])
