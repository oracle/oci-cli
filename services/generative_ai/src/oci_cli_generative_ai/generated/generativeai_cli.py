...
@click.command(cli_util.override('generative_ai.model_discovery_collection_group.command_name', 'model-discovery-collection'), cls=CommandGroupWithAlias, help="""A paginated collection of model discovery results. Each item represents a model along with its capabilities and regional availability.""")
@cli_util.help_option_group
def model_discovery_collection_group():
    pass
...
@click.command(cli_util.override('generative_ai.routing_profile_collection_group.command_name', 'routing-profile-collection'), cls=CommandGroupWithAlias, help="""Results of a routing profile search. Contains RoutingProfileSummary items and other information such as metadata.""")
@cli_util.help_option_group
def routing_profile_collection_group():
    pass
...
@click.command(cli_util.override('generative_ai.routing_profile_group.command_name', 'routing-profile'), cls=CommandGroupWithAlias, help="""A routing profile is a reusable set of policies that controls how Smart Router selects a target region and model for inference traffic. It can scope routing to allowed regions and allowed models.

To use any of the API operations, you must be authorized in an IAM policy. If you're not authorized, talk to an administrator who gives OCI resource access to users. See [Getting Started with Policies] and [Getting Access to Generative AI Resources].""")
@cli_util.help_option_group
def routing_profile_group():
    pass
...
generative_ai_root_group.add_command(model_discovery_collection_group)
generative_ai_root_group.add_command(endpoint_group)
generative_ai_root_group.add_command(semantic_store_group)
generative_ai_root_group.add_command(routing_profile_collection_group)
...
generative_ai_root_group.add_command(routing_profile_group)
...
@routing_profile_group.command(name=cli_util.override('generative_ai.change_routing_profile_compartment.command_name', 'change-compartment'), help=u"""Moves a routing profile into a different compartment within the same tenancy. For information about moving resources between compartments, see [Moving Resources to a Different Compartment]. \n[Command Reference](changeRoutingProfileCompartment)""")
@cli_util.option('--routing-profile-id', required=True, help=u"""The [OCID] of the routing profile.""")
@cli_util.option('--compartment-id', required=True, help=u"""The OCID of the compartment to move the routing profile to.""")
@cli_util.option('--if-match', help=u"""For optimistic concurrency control. In the PUT or DELETE call for a resource, set the `if-match` parameter to the value of the etag from a previous GET or POST response for that resource. The resource will be updated or deleted only if the etag you provide matches the resource's current etag value.""")
@json_skeleton_utils.get_cli_json_input_option({})
@cli_util.help_option
@click.pass_context
@json_skeleton_utils.json_skeleton_generation_handler(input_params_to_complex_types={})
@cli_util.wrap_exceptions
def change_routing_profile_compartment(ctx, from_json, routing_profile_id, compartment_id, if_match):

    if isinstance(routing_profile_id, six.string_types) and len(routing_profile_id.strip()) == 0:
        raise click.UsageError('Parameter --routing-profile-id cannot be whitespace or empty string')
    ...
    client = cli_util.build_client('generative_ai', 'generative_ai', ctx)
    result = client.change_routing_profile_compartment(
        routing_profile_id=routing_profile_id,
        change_routing_profile_compartment_details=_details,
        **kwargs
    )
    cli_util.render_response(result, ctx)
...
@routing_profile_group.command(name=cli_util.override('generative_ai.create_routing_profile.command_name', 'create'), help=u"""Creates a routing profile. \n[Command Reference](createRoutingProfile)""")
@cli_util.option('--display-name', required=True, help=u"""A user-friendly name. Does not have to be unique, and it's changeable.""")
@cli_util.option('--compartment-id', required=True, help=u"""The OCID of the compartment in which to create the routing profile.""")
@cli_util.option('--description', help=u"""An optional description of the routing profile.""")
@cli_util.option('--region-routing-policy', type=custom_types.CLI_COMPLEX_TYPE, help=u"""""" + custom_types.cli_complex_type.COMPLEX_TYPE_HELP)
@cli_util.option('--model-routing-policy', type=custom_types.CLI_COMPLEX_TYPE, help=u"""""" + custom_types.cli_complex_type.COMPLEX_TYPE_HELP)
@cli_util.option('--freeform-tags', type=custom_types.CLI_COMPLEX_TYPE, help=u"""Free-form tags for this resource. Each tag is a simple key-value pair with no predefined name, type, or namespace. For more information, see [Resource Tags].
...
@cli_util.option('--max-wait-seconds', type=click.INT, help="""The maximum time to wait for the resource to reach the lifecycle state defined by --wait-for-state. Defaults to 1200 seconds.""")
@cli_util.option('--wait-interval-seconds', type=click.INT, help="""Check every --wait-interval-seconds to see whether the resource has reached the lifecycle state defined by --wait-for-state. Defaults to 30 seconds.""")
@json_skeleton_utils.get_cli_json_input_option({'region-routing-policy': {'module': 'generative_ai', 'class': 'RegionRoutingPolicy'}, 'model-routing-policy': {'module': 'generative_ai', 'class': 'ModelRoutingPolicy'}, 'freeform-tags': {'module': 'generative_ai', 'class': 'dict(str, string)'}, 'defined-tags': {'module': 'generative_ai', 'class': 'dict(str, dict(str, object))'}})
@cli_util.help_option
@click.pass_context
...
@cli_util.wrap_exceptions
def create_routing_profile(ctx, from_json, wait_for_state, max_wait_seconds, wait_interval_seconds, display_name, compartment_id, description, region_routing_policy, model_routing_policy, freeform_tags, defined_tags):
    ...
    if region_routing_policy is not None:
        _details['regionRoutingPolicy'] = cli_util.parse_json_parameter("region_routing_policy", region_routing_policy)

    if model_routing_policy is not None:
        _details['modelRoutingPolicy'] = cli_util.parse_json_parameter("model_routing_policy", model_routing_policy)
    ...
    client = cli_util.build_client('generative_ai', 'generative_ai', ctx)
    result = client.create_routing_profile(
        create_routing_profile_details=_details,
        **kwargs
    )
    if wait_for_state:

        if hasattr(client, 'get_routing_profile') and callable(getattr(client, 'get_routing_profile')):
            try:
                wait_period_kwargs = {}
                if max_wait_seconds is not None:
                    wait_period_kwargs['max_wait_seconds'] = max_wait_seconds
                ...
                click.echo('Action completed. Waiting until the resource has entered state: {}'.format(wait_for_state), file=sys.stderr)
                result = oci.wait_until(client, client.get_routing_profile(result.data.id), 'lifecycle_state', wait_for_state, **wait_period_kwargs)
            except oci.exceptions.MaximumWaitTimeExceeded as e:
                # If we fail, we should show an error, but we should still provide the information to the customer
                click.echo('Failed to wait until the resource entered the specified state. Outputting last known resource state', file=sys.stderr)
                cli_util.render_response(result, ctx)
...
@routing_profile_group.command(name=cli_util.override('generative_ai.delete_routing_profile.command_name', 'delete'), help=u"""Deletes a routing profile. \n[Command Reference](deleteRoutingProfile)""")
@cli_util.option('--routing-profile-id', required=True, help=u"""The [OCID] of the routing profile.""")
@cli_util.option('--if-match', help=u"""For optimistic concurrency control. In the PUT or DELETE call for a resource, set the `if-match` parameter to the value of the etag from a previous GET or POST response for that resource. The resource will be updated or deleted only if the etag you provide matches the resource's current etag value.""")
@cli_util.confirm_delete_option
@cli_util.option('--wait-for-state', type=custom_types.CliCaseInsensitiveChoice(["ACTIVE", "CREATING", "UPDATING", "DELETING", "DELETED", "FAILED"]), multiple=True, help="""This operation creates, modifies or deletes a resource that has a defined lifecycle state. Specify this option to perform the action and then wait until the resource reaches a given lifecycle state. Multiple states can be specified, returning on the first state. For example, --wait-for-state ACTIVE --wait-for-state FAILED would return on whichever lifecycle state is reached first. If timeout is reached, a return code of 2 is returned. For any other error, a return code of 1 is returned.""")
...
@json_skeleton_utils.json_skeleton_generation_handler(input_params_to_complex_types={})
@cli_util.wrap_exceptions
def delete_routing_profile(ctx, from_json, wait_for_state, max_wait_seconds, wait_interval_seconds, routing_profile_id, if_match):
    ...
    kwargs['opc_request_id'] = cli_util.use_or_generate_request_id(ctx.obj['request_id'])
    client = cli_util.build_client('generative_ai', 'generative_ai', ctx)
    result = client.delete_routing_profile(
        routing_profile_id=routing_profile_id,
        **kwargs
    )
    if wait_for_state:
        ...
        if hasattr(client, 'get_routing_profile') and callable(getattr(client, 'get_routing_profile')):
            try:
                ...
                click.echo('Action completed. Waiting until the resource has entered state: {}'.format(wait_for_state), file=sys.stderr)
                oci.wait_until(client, client.get_routing_profile(routing_profile_id), 'lifecycle_state', wait_for_state, succeed_on_not_found=True, **wait_period_kwargs)
            except oci.exceptions.ServiceError as e:
                # We make an initial service call so we can pass the result to oci.wait_until(), however if we are waiting on the
                # outcome of a delete operation it is possible that the resource is already gone and so the initial service call
                # will result in an exception that reflects a HTTP 404. In this case, we can exit with success (rather than raising
...
@routing_profile_group.command(name=cli_util.override('generative_ai.get_routing_profile.command_name', 'get'), help=u"""Gets information about a routing profile. \n[Command Reference](getRoutingProfile)""")
@cli_util.option('--routing-profile-id', required=True, help=u"""The [OCID] of the routing profile.""")
@json_skeleton_utils.get_cli_json_input_option({})
@cli_util.help_option
@click.pass_context
@json_skeleton_utils.json_skeleton_generation_handler(input_params_to_complex_types={}, output_type={'module': 'generative_ai', 'class': 'RoutingProfile'})
@cli_util.wrap_exceptions
def get_routing_profile(ctx, from_json, routing_profile_id):
    ...
    kwargs['opc_request_id'] = cli_util.use_or_generate_request_id(ctx.obj['request_id'])
    client = cli_util.build_client('generative_ai', 'generative_ai', ctx)
    result = client.get_routing_profile(
        routing_profile_id=routing_profile_id,
        **kwargs
    )
    cli_util.render_response(result, ctx)
...
@model_discovery_collection_group.command(name=cli_util.override('generative_ai.list_model_discovery.command_name', 'list-model-discovery'), help=u"""Retrieves a list of models along with their capabilities, supported features, and deployment availability. Results can be filtered by attributes such as region, realm, model identifier, supported inference APIs, serving modes, and access type. \n[Command Reference](listModelDiscovery)""")
@cli_util.option('--compartment-id', required=True, help=u"""The [OCID] of the compartment in which to list resources.""")
@cli_util.option('--realm', multiple=True, help=u"""A filter to return only resources whose realm matches the given realm.""")
@cli_util.option('--region-parameterconflict', multiple=True, help=u"""A filter to return only resources whose region matches the given region.""")
@cli_util.option('--model-id', help=u"""A filter to return only resources whose model identifier matches the given modelId.""")
@cli_util.option('--serving-mode', type=custom_types.CliCaseInsensitiveChoice(["ON_DEMAND", "DEDICATED"]), multiple=True, help=u"""A filter to return only resources whose serving modes match the given servingModes.""")
@cli_util.option('--api-capability', multiple=True, help=u"""Filter models that support any of the specified API capabilities.""")
...
@cli_util.option('--model-access', type=custom_types.CliCaseInsensitiveChoice(["HOSTED", "PROXY"]), multiple=True, help=u"""Filter models by access type.""")
@cli_util.option('--is-deprecated', type=click.BOOL, help=u"""If true, return only deprecated models; if false, exclude deprecated models.""")
@cli_util.option('--is-on-demand-retired', type=click.BOOL, help=u"""Filter models based on on-demand retirement status.""")
@cli_util.option('--is-dedicated-retired', type=click.BOOL, help=u"""Filter models based on dedicated retirement status.""")
@cli_util.option('--limit', type=click.INT, help=u"""For list pagination. The maximum number of results per page, or items to return in a paginated \"List\" call. For important details about how pagination works, see [List Pagination].""")
@cli_util.option('--page', help=u"""For list pagination. The value of the opc-next-page response header from the previous \"List\" call. For important details about how pagination works, see [List Pagination].""")
@cli_util.option('--all', 'all_pages', is_flag=True, help="""Fetches all pages of results. If you provide this option, then you cannot provide the --limit option.""")
@cli_util.option('--page-size', type=click.INT, help="""When fetching results, the number of results to fetch per call. Only valid when used with --all or --limit, and ignored otherwise.""")
@json_skeleton_utils.get_cli_json_input_option({'realm': {'module': 'generative_ai', 'class': 'list[string]'}, 'region-parameterconflict': {'module': 'generative_ai', 'class': 'list[string]'}, 'api-capability': {'module': 'generative_ai', 'class': 'list[string]'}})
@cli_util.help_option
@click.pass_context
@json_skeleton_utils.json_skeleton_generation_handler(input_params_to_complex_types={'realm': {'module': 'generative_ai', 'class': 'list[string]'}, 'region-parameterconflict': {'module': 'generative_ai', 'class': 'list[string]'}, 'api-capability': {'module': 'generative_ai', 'class': 'list[string]'}}, output_type={'module': 'generative_ai', 'class': 'ModelDiscoveryCollection'})
@cli_util.wrap_exceptions
def list_model_discovery(ctx, from_json, all_pages, page_size, compartment_id, realm, region_parameterconflict, model_id, serving_mode, api_capability, capability, model_access, is_deprecated, is_on_demand_retired, is_dedicated_retired, limit, page):
    ...
    if realm is not None and len(realm) > 0:
        kwargs['realm'] = realm
    if region_parameterconflict is not None and len(region_parameterconflict) > 0:
        kwargs['region'] = region_parameterconflict
    if model_id is not None:
        kwargs['model_id'] = model_id
    if serving_mode is not None and len(serving_mode) > 0:
        kwargs['serving_mode'] = serving_mode
    if api_capability is not None and len(api_capability) > 0:
        kwargs['api_capability'] = api_capability
    if capability is not None and len(capability) > 0:
        kwargs['capability'] = capability
    if model_access is not None and len(model_access) > 0:
        kwargs['model_access'] = model_access
    if is_deprecated is not None:
        kwargs['is_deprecated'] = is_deprecated
    if is_on_demand_retired is not None:
        kwargs['is_on_demand_retired'] = is_on_demand_retired
    if is_dedicated_retired is not None:
        kwargs['is_dedicated_retired'] = is_dedicated_retired
    if limit is not None:
        kwargs['limit'] = limit
    if page is not None:
        kwargs['page'] = page
    ...
    if all_pages:
        ...
        result = client.list_model_discovery(
            compartment_id=compartment_id,
            **kwargs
        )
    cli_util.render_response(result, ctx)
...
@routing_profile_collection_group.command(name=cli_util.override('generative_ai.list_routing_profiles.command_name', 'list-routing-profiles'), help=u"""Lists the routing profiles of a specific compartment. \n[Command Reference](listRoutingProfiles)""")
@cli_util.option('--compartment-id', required=True, help=u"""The [OCID] of the compartment in which to list resources.""")
@cli_util.option('--lifecycle-state', type=custom_types.CliCaseInsensitiveChoice(["ACTIVE", "CREATING", "UPDATING", "DELETING", "DELETED", "FAILED"]), help=u"""A filter to return only resources whose lifecycle state matches the given value.""")
@cli_util.option('--display-name', help=u"""A filter to return only resources that match the given display name exactly.""")
@cli_util.option('--id', help=u"""The [OCID] of the routing profile.""")
@cli_util.option('--limit', type=click.INT, help=u"""For list pagination. The maximum number of results per page, or items to return in a paginated \"List\" call. For important details about how pagination works, see [List Pagination].""")
@cli_util.option('--page', help=u"""For list pagination. The value of the opc-next-page response header from the previous \"List\" call. For important details about how pagination works, see [List Pagination].""")
@cli_util.option('--sort-order', type=custom_types.CliCaseInsensitiveChoice(["ASC", "DESC"]), help=u"""The sort order to use, either 'ASC' or 'DESC'.""")
@cli_util.option('--sort-by', type=custom_types.CliCaseInsensitiveChoice(["displayName", "timeCreated"]), help=u"""The field to sort by. You can provide only one sort order. Default order for `timeCreated` is descending. Default order for `displayName` is ascending.""")
...
@cli_util.help_option
@click.pass_context
@json_skeleton_utils.json_skeleton_generation_handler(input_params_to_complex_types={}, output_type={'module': 'generative_ai', 'class': 'RoutingProfileCollection'})
@cli_util.wrap_exceptions
def list_routing_profiles(ctx, from_json, all_pages, page_size, compartment_id, lifecycle_state, display_name, id, limit, page, sort_order, sort_by):
    ...
    if all_pages:
        ...
        result = client.list_routing_profiles(
            compartment_id=compartment_id,
            **kwargs
        )
    cli_util.render_response(result, ctx)
...
@routing_profile_group.command(name=cli_util.override('generative_ai.update_routing_profile.command_name', 'update'), help=u"""Updates the properties of a routing profile. \n[Command Reference](updateRoutingProfile)""")
@cli_util.option('--routing-profile-id', required=True, help=u"""The [OCID] of the routing profile.""")
@cli_util.option('--display-name', help=u"""A user-friendly name. Does not have to be unique, and it's changeable.""")
@cli_util.option('--description', help=u"""An optional description of the routing profile.""")
@cli_util.option('--region-routing-policy', type=custom_types.CLI_COMPLEX_TYPE, help=u"""""" + custom_types.cli_complex_type.COMPLEX_TYPE_HELP)
...
@json_skeleton_utils.json_skeleton_generation_handler(input_params_to_complex_types={'region-routing-policy': {'module': 'generative_ai', 'class': 'RegionRoutingPolicy'}, 'model-routing-policy': {'module': 'generative_ai', 'class': 'ModelRoutingPolicy'}, 'freeform-tags': {'module': 'generative_ai', 'class': 'dict(str, string)'}, 'defined-tags': {'module': 'generative_ai', 'class': 'dict(str, dict(str, object))'}}, output_type={'module': 'generative_ai', 'class': 'RoutingProfile'})
@cli_util.wrap_exceptions
def update_routing_profile(ctx, from_json, force, wait_for_state, max_wait_seconds, wait_interval_seconds, routing_profile_id, display_name, description, region_routing_policy, model_routing_policy, freeform_tags, defined_tags, if_match):

    if isinstance(routing_profile_id, six.string_types) and len(routing_profile_id.strip()) == 0:
        raise click.UsageError('Parameter --routing-profile-id cannot be whitespace or empty string')
    if not force:
        if region_routing_policy or model_routing_policy or freeform_tags or defined_tags:
            if not click.confirm("WARNING: Updates to region-routing-policy and model-routing-policy and freeform-tags and defined-tags will replace any existing values. Are you sure you want to continue?"):
                ctx.abort()
    ...
    client = cli_util.build_client('generative_ai', 'generative_ai', ctx)
    result = client.update_routing_profile(
        routing_profile_id=routing_profile_id,
        update_routing_profile_details=_details,
        **kwargs
    )
    if wait_for_state:
