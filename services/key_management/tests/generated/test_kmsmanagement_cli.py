...
def test_cancel_key_deletion(cli_testing_service_client, runner, config_file, config_profile):
    if not cli_testing_service_client.is_api_enabled('key_management', 'CancelKeyDeletion'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config_file = os.environ['OCI_CLI_CONFIG_FILE']
    if 'USE_TESTING_SERVICE_CONFIG' in os.environ:
        try:
            config_str = cli_testing_service_client.get_config('key_management', 'KmsManagement', 'CancelKeyDeletion')
            config = json.loads(config_str)
            key_file_content = config['keyFileContent']
            with open('tests/resources/keyfile_for_test_cancel_key_deletion.pem', 'w') as f:
                f.write(key_file_content)
            with open('tests/resources/config_for_test_cancel_key_deletion', 'w') as f:
                f.write('[ADMIN]\n')
                ...
                f.write('key_file = tests/resources/keyfile_for_test_cancel_key_deletion.pem\n')
            runner.invoke(oci_cli.cli, ['setup', 'repair-file-permissions', '--file', 'tests/resources/config_for_test_cancel_key_deletion'])
            runner.invoke(oci_cli.cli, ['setup', 'repair-file-permissions', '--file', 'tests/resources/keyfile_for_test_cancel_key_deletion.pem'])
            config_file = 'tests/resources/config_for_test_cancel_key_deletion'
    ...
    request_containers = cli_testing_service_client.get_requests(service_name='key_management', api_name='CancelKeyDeletion')
    for i in range(len(request_containers)):
        ...
        while not done:
            ...
            request, cleanup = generated_test_request_transformers.transform_generated_test_input('key_management', 'CancelKeyDeletion', request)
            ...
            if not params:
                params = util.get_command_list(
                    ...
                    oci_cli.cli_util.override('cancel_key_deletion.command_name', 'cancel-key-deletion'),
                    True
                )

            if not params:
                raise ValueError(
                    'Failed to find CLI command "oci {} {} {}" for given operation: key_management, CancelKeyDeletion. '
            ...
            params.append(cli_testing_service_client.get_endpoint("key_management", "KmsManagementClient", "CancelKeyDeletion"))
            ...
            try:
                ...
                if os.path.exists('tests/resources/keyfile_for_test_cancel_key_deletion.pem'):
                    os.remove('tests/resources/keyfile_for_test_cancel_key_deletion.pem')
                if os.path.exists('tests/resources/config_for_test_cancel_key_deletion'):
                    os.remove('tests/resources/config_for_test_cancel_key_deletion')
                if cleanup:
                    try:
                        next(cleanup)
...
@pytest.mark.generated
def test_schedule_key_deletion(cli_testing_service_client, runner, config_file, config_profile):
    if not cli_testing_service_client.is_api_enabled('key_management', 'ScheduleKeyDeletion'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config_file = os.environ['OCI_CLI_CONFIG_FILE']
    if 'USE_TESTING_SERVICE_CONFIG' in os.environ:
        try:
            config_str = cli_testing_service_client.get_config('key_management', 'KmsManagement', 'ScheduleKeyDeletion')
            config = json.loads(config_str)
            key_file_content = config['keyFileContent']
            with open('tests/resources/keyfile_for_test_schedule_key_deletion.pem', 'w') as f:
                f.write(key_file_content)
            with open('tests/resources/config_for_test_schedule_key_deletion', 'w') as f:
                f.write('[ADMIN]\n')
                f.write('user = ' + config['userId'] + '\n')
                f.write('fingerprint = ' + config['fingerprint'] + '\n')
                f.write('tenancy = ' + config['tenantId'] + '\n')
                f.write('region = ' + config['region'] + '\n')
                f.write('key_file = tests/resources/keyfile_for_test_schedule_key_deletion.pem\n')
            runner.invoke(oci_cli.cli, ['setup', 'repair-file-permissions', '--file', 'tests/resources/config_for_test_schedule_key_deletion'])
            runner.invoke(oci_cli.cli, ['setup', 'repair-file-permissions', '--file', 'tests/resources/keyfile_for_test_schedule_key_deletion.pem'])
            config_file = 'tests/resources/config_for_test_schedule_key_deletion'
        except vcr.errors.CannotOverwriteExistingCassetteException:
            pass
        except Exception as e:
            print(e)
    ...
    root_command_name = oci_cli.cli_util.override('kms_management_root_group.command_name', 'kms-management')
    resource_group_command_name = oci_cli.cli_util.override('key_group.command_name', 'key')
    request_containers = cli_testing_service_client.get_requests(service_name='key_management', api_name='ScheduleKeyDeletion')
    for i in range(len(request_containers)):
        request = request_containers[i]['request'].copy()
        done = False
        params = []
        while not done:
            # force all details param names to have lower case first letter for consistency with Java models
            param_name = 'ScheduleKeyDeletionDetails'
            details = request.pop(param_name[0].lower() + param_name[1:])
            for key in details:
                request[key] = details[key]
                override = util.variable_name_override(key)
            ...
            request, cleanup = generated_test_request_transformers.transform_generated_test_input('key_management', 'ScheduleKeyDeletion', request)
            ...
            if not params:
                params = util.get_command_list(
                    root_command_name,
                    resource_group_command_name,
                    oci_cli.cli_util.override('schedule_key_deletion.command_name', 'schedule-key-deletion'),
                    True
                )

            if not params:
                raise ValueError(
                    'Failed to find CLI command "oci {} {} {}" for given operation: key_management, ScheduleKeyDeletion. '
                    'This usually happens because generated commands have been manually re-arranged in code for better user '
                    'experience. To allow this test to find the proper command, please add an entry to MOVED_COMMANDS in '
                    'services/<spec_name>/tests/extend_test_<your_service_name>.py to map ({}, {}, {}) to the syntax '
                    'for the new command. If the file does not exist for your service, please create one. You can refer the '
            ...
            params.append('--endpoint')
            params.append(cli_testing_service_client.get_endpoint("key_management", "KmsManagementClient", "ScheduleKeyDeletion"))
            ...
            try:
                ...
                if os.path.exists('tests/resources/keyfile_for_test_schedule_key_deletion.pem'):
                    os.remove('tests/resources/keyfile_for_test_schedule_key_deletion.pem')
                if os.path.exists('tests/resources/config_for_test_schedule_key_deletion'):
                    os.remove('tests/resources/config_for_test_schedule_key_deletion')
                if cleanup:
                    try:
                        next(cleanup)
                    except StopIteration:
