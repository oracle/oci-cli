...
@click.command(cli_util.override('esxi_host.esxi_host_group.command_name', 'esxi-host'), cls=CommandGroupWithAlias, help="""An ESXi host is a node in a Cluster. At a minimum, each Cluster has 3 ESXi hosts that are used to implement a functioning VMware environment.
...
@cli_util.option('--current-commitment', type=custom_types.CliCaseInsensitiveChoice(["HOUR", "MONTH", "ONE_YEAR", "THREE_YEARS"]), help=u"""The billing option currently used by the ESXi host. [ListSupportedCommitments].""")
...
def create_esxi_host(ctx, from_json, wait_for_state, max_wait_seconds, wait_interval_seconds, cluster_id, display_name, billing_donor_host_id, current_commitment, next_commitment, compute_availability_domain, host_shape_name, host_ocpu_count, capacity_reservation_id, esxi_software_version, vcf_byol_allocation_id, is_vsan_byol_enabled, freeform_tags, defined_tags):
    ...
    if current_commitment is not None:
        _details['currentCommitment'] = current_commitment

    if next_commitment is not None:
        _details['nextCommitment'] = next_commitment
...
def inplace_upgrade(ctx, from_json, wait_for_state, max_wait_seconds, wait_interval_seconds, esxi_host_id, vcf_byol_allocation_id, if_match):
    ...
    client = cli_util.build_client('ocvp', 'esxi_host', ctx)
    result = client.inplace_upgrade(
        esxi_host_id=esxi_host_id,
        inplace_upgrade_details=_details,
        **kwargs
    )
    if wait_for_state:
...
@cli_util.wrap_exceptions
def list_esxi_hosts(ctx, from_json, all_pages, page_size, sddc_id, cluster_id, compute_instance_id, display_name, limit, page, sort_order, sort_by, lifecycle_state, is_billing_donors_only, is_swap_billing_only, compartment_id):
...
def replace_host(ctx, from_json, wait_for_state, max_wait_seconds, wait_interval_seconds, esxi_host_id, esxi_software_version, vcf_byol_allocation_id, if_match):
    ...
    client = cli_util.build_client('ocvp', 'esxi_host', ctx)
    result = client.replace_host(
        esxi_host_id=esxi_host_id,
        replace_host_details=_details,
        **kwargs
    )
    if wait_for_state:
